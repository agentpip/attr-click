...
- SQLite for local development; MySQL 8-compatible production deployment
...
- Production is served by Caddy at `https://attr.click`; repository-local Envoy requires local `ATTR_CLICK_DEPLOY_HOST` and `ATTR_CLICK_DEPLOY_PATH` values, builds Vite assets locally, syncs assets before source, migrates, optimizes, and probes `/up` without restarting a service.
