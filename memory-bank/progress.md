...
- PR #11 merged the MIT/open-source public-link release as `80e4dc8`; Envoy deployed it from `origin/main` on 2026-07-25. The root LICENSE is standard MIT text and GitHub reports MIT license recognition. The homepage footer and public `/help` page both expose canonical source and license links. Public link inspection, source/license URL checks, `/`, `/help`, `/up`, `/login`, guest-auth redirects, loopback health/help, and deployed source hashes all passed.
- PR #9 merged the canonical-repository link update as `6031505`; Envoy deployed it from `origin/main` on 2026-07-25. The canonical public repository is `msitarzewski/attr-click`; local `origin` and public `/help` links use that URL. The production directory is an rsync artifact with no Git remote, so it is not a fork. Public canonical-link inspection, `/`, `/help`, `/up`, `/login`, guest-auth redirects, loopback health/help, and deployed source hashes all passed.
