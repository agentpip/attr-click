<x-layout>
    ...
    <div class="mt-10 grid gap-8 lg:grid-cols-[1fr_360px]">
        <div class="grid gap-4 sm:grid-cols-2">@forelse ($templates as $template)
            <div class="rounded-2xl border border-zinc-200 bg-white p-5 shadow-sm dark:border-zinc-800 dark:bg-zinc-900/70"><div class="h-16 rounded-xl" style="background: {{ $template->background_color }}; border: 12px solid {{ $template->foreground_color }}"></div><h2 class="mt-4 font-bold">{{ $template->name }}</h2><p class="mt-1 font-mono text-xs text-zinc-600 dark:text-zinc-400">{{ $template->foreground_color }} / {{ $template->background_color }}</p></div>
        @empty <p class="text-slate-400">No saved templates yet.</p>@endforelse</div>
        <form method="POST" action="{{ route('templates.store') }}" class="rounded-2xl border border-zinc-200 bg-white p-6 shadow-sm dark:border-zinc-800 dark:bg-zinc-900/70 space-y-5">@csrf
